...
const ValidarInformacion = (data: Documento[]) => {
    mockDocumentosErrores.map(docError => {
        const docIndex = data.findIndex(docData => docData?.Numeracion == docError?.Numeracion)
                ...
                Declarar(dataEnvio)
                    .then((rta: any) => {
                        ...
                        data.map((documento: RespuestaServicio) => {
                            ...
                            const indexDoc = dataEnvio.findIndex(documentoMock => `${documentoMock.Numeracion}-0${documentoMock.TipoDocumento}` == documento.documento)
